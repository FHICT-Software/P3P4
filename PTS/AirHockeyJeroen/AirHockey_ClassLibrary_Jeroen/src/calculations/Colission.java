 ...
 * @date 14/04/02
 */
public class Colission implements Serializable {
    ...
    private static void intersectsWithBat(Bat bat) {
        ...
        if (Math.sqrt(Math.pow(batX - puckX, 2) + Math.pow(puckY - batY, 2)) < batSize + puckSize + MARGIN) {
            ...
            if (batX > puckX && batY > puckY) {
                ...
                puckX = (int) (batX - (Math.sin(Math.toRadians(hoek)) * (batSize + puckSize)));
                puckY = (int) (batY - (Math.cos(Math.toRadians(hoek)) * (batSize + puckSize)));
            } //</editor-fold>
            //<editor-fold defaultstate="collapsed" desc="Q2">
            else if (batX > puckX && puckY > batY) {
                hoek = 180 - Math.toDegrees(Math.atan2(batX - puckX, puckY - batY));
                puckX = (int) (batX - (Math.sin(Math.toRadians(hoek)) * (batSize + puckSize)));
                puckY = (int) ((Math.cos(Math.toRadians(hoek)) * (batSize + puckSize)) + batY);
            } //</editor-fold>
            //<editor-fold defaultstate="collapsed" desc="Q3">
                ...
                puckX = (int) ((Math.sin(Math.toRadians(hoek)) * (batSize + puckSize)) + batX);
                puckY = (int) ((Math.cos(Math.toRadians(hoek)) * (batSize + puckSize)) + batY);
            } //</editor-fold>
            //<editor-fold defaultstate="collapsed" desc="Q4">
            else if (puckX > batX && batY > puckY) {
                hoek = 0 - Math.toDegrees(Math.atan2(puckX - batX, batY - puckY));
                puckX = (int) ((Math.sin(Math.toRadians(hoek)) * (batSize + puckSize)) + batX);
                puckY = (int) (batY - (Math.cos(Math.toRadians(hoek)) * (batSize + puckSize)));
            }
            //</editor-fold>
